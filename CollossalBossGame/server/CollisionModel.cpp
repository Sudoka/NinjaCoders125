...
bool areColliding(Vec3f *shift, DIRECTION *collDir, const Box &bx, const Point_t &hmapCenter, const HMapElement &hmap) {
	...
	*collDir = hmap.dir;
	...
	case NORTH:
		...
		break;
	case SOUTH:
		objPos = Point_t(-(bx.x + bx.w / 2), -(bx.z + bx.l), bx.y + bx.h / 2);	//Back of the box
		hmapPos = Point_t(-hmapCenter.x, -hmapCenter.z, hmapCenter.y);
		break;
	case EAST:
		objPos =  Point_t((bx.z + bx.l / 2), bx.x, bx.y + bx.h / 2);	//right side of the box
		hmapPos = Point_t(hmapCenter.z, hmapCenter.x, hmapCenter.y);
		break;
	case WEST:
		objPos =  Point_t((bx.z + bx.l / 2), -(bx.x + bx.w), bx.y + bx.h / 2);	//right side of the box
		hmapPos = Point_t(hmapCenter.z, -hmapCenter.x, hmapCenter.y);
		break;
	case DOWN:
		objPos = Point_t(-(bx.x + bx.w / 2), -(bx.y + bx.h), bx.z + bx.l / 2);	//Bottom of the box
